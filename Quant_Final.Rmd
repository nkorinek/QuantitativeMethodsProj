...
for (i in 1:1){
  ...
  keep_y <- ndvi_list_post[i]
  print(c("target:", keep_y))
  
  keep <- c(keep_attr_pre, keep_attr_post, static_list,keep_y)
  ...
  rf_model <- randomForest(T1after ~ ., data = current, importance = TRUE, ntree=500,mtry=50,type='regression')
